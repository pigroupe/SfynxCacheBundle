 ...
 * @since      2015-02-16
 */
class SerializerHandlerPass extends AbstractHandlerPass
{
    /** @var string */
    const HANDLER_NAME = FactoryPassInterface::HANDLER_SERIALIZER;
    ...
    protected function process(ContainerBuilder $container, array $config): bool
    {
        if (!empty($config['provider'])) {
            ...
            $container->register(sprintf('sfynx.cache.%s', static::HANDLER_NAME), DoctrineCacheBridge::class)
                ->setFactory([SerializerHandlerPass::class, 'getBridge'])
        ...
        if (!empty($config['factory'])) {
            ...
            $container->register(sprintf('sfynx.cache.%s', static::HANDLER_NAME), DoctrineCacheBridge::class)
                ->setFactory([SerializerHandlerPass::class, 'getBridgeFactory'])
                ->addArgument($factory)
                ->addArgument($config)
                ->addArgument([static::HANDLER_NAME]);
        }
